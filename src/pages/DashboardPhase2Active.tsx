...
const acc = {
  ...
  maxDDUsedPct: 8,
  balanceHistory: [
    { day: 'Jan 18', balance: 100000 },
    { day: 'Jan 19', balance: 100580 },
    { day: 'Jan 20', balance: 100320 },
    { day: 'Jan 21', balance: 100890 },
    { day: 'Jan 22', balance: 101240 },
  ],
}
...
export function DashboardPhase2Active() {
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', background: 'linear-gradient(135deg, #0ea5e9, #38bdf8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>PHASE 2</div>
        ...
        <button style={{ display: 'flex', alignItems: 'center', gap: 7, background: 'linear-gradient(135deg, #0ea5e9 0%, #6366f1 100%)', border: 'none', borderRadius: 7, padding: '8px 16px', color: '#fff', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>
        ...
        <div style={{ background: '#0f0f1c', border: '1px solid #1a1a28', borderRadius: 10, padding: '20px' }}>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <AreaChart data={acc.balanceHistory}>
              ...
              <Area type="monotone" dataKey="balance" stroke="#0ea5e9" strokeWidth={2} fill="url(#p2Grad)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ background: '#0f0f1c', border: '1px solid #1a1a28', borderRadius: 10, padding: '18px 20px', flex: 1 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: '#f1f5f9', marginBottom: 4 }}>
              ${fmt(acc.totalPnl, 0)} <span style={{ fontSize: 13, color: '#334155', fontWeight: 400 }}>/ ${fmt(profitTargetAmt, 0)}</span>
            </div>
            <div style={{ fontSize: 12, color: '#64748b', marginBottom: 10 }}>{profitProgress.toFixed(1)}% of {acc.profitTargetPct}% target</div>
            <ProgressBar value={profitProgress} color="#0ea5e9" />
          </div>
          <div style={{ background: '#0f0f1c', border: '1px solid #1a1a28', borderRadius: 10, padding: '18px 20px', flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {Array.from({ length: acc.requiredTradingDays }).map((_, i) => (
                <div key={i} style={{ height: 28, flex: 1, borderRadius: 5, background: i < acc.tradingDays ? '#0ea5e9' : '#1a1a28' }} />
              ))}
            </div>
